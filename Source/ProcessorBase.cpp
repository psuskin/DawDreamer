...
void ProcessorBase::numChannelsChanged() { m_isConnectedInGraph = false; }
...
py::dict ProcessorBase::getAutomationAll() {
  ...
  for (auto it = m_recordedAutomationDict.begin();
       it != m_recordedAutomationDict.end(); it++) {
...
void ProcessorBase::recordAutomation(AudioPlayHead::PositionInfo& posInfo,
                                     int numSamples) {
...
void ProcessorBase::processBlock(juce::AudioSampleBuffer& buffer,
                                 juce::MidiBuffer&) {
  if (!m_recordEnable) {
    return;
  }
  ...
  int numSamplesToCopy =
      std::min(buffer.getNumSamples(), myRecordBuffer.getNumSamples() -
  ...
  for (int chan = 0; chan < numberChannels; chan++) {
    ...
    myRecordBuffer.copyFrom(chan, writePos, buffer.getReadPointer(chan),
                            numSamplesToCopy);
  }
}

py::array_t<float> ProcessorBase::bufferToPyArray(
    juce::AudioSampleBuffer& buffer) {
