...
class ProcessorBase : public juce::AudioProcessor {
 public:
  ...
  ProcessorBase(std::string newUniqueName = "")
      : AudioProcessor{}, myUniqueName{newUniqueName} {
  ...
  // All subclasses of ProcessorBase should implement processBlock and call
  // ProcessorBase::processBlock at the end of their implementation. The
  // ProcessorBase implementation takes care of recording.
  ...
  //==============================================================================
  virtual bool canApplyBusesLayout(
  ...
  virtual bool setAutomation(std::string& parameterName, py::array input,
                             std::uint32_t ppqn);
  ...
  virtual void automateParameters(AudioPlayHead::PositionInfo& posInfo,
                                  int numSamples){};
  ...
  void setRecordAutomationEnable(bool recordAutomation) {
    m_recordAutomation = recordAutomation;
  }
  ...
  virtual int getTotalNumOutputChannels() {
    return AudioProcessor::getTotalNumOutputChannels();
  }

  virtual int getTotalNumInputChannels() {
  ...
  bool setMainBusInputsAndOutputs(int inputs, int outputs) {
    ...
    if (this->canApplyBusesLayout(busesLayout)) {
      ...
      throw std::invalid_argument(
          this->getUniqueName() + " CANNOT ApplyBusesLayout inputs: " +
          std::to_string(inputs) + " outputs: " + std::to_string(outputs));
    }
  }
  ...
  // It's easy to "upsample" to this higher resolution, as we do in `load_midi`
  // and `add_midi_note`. Another bad design of the code is that FaustProcessor,
  // PluginProcessor, and SamplerProcessor have a lot of common code related to
  // MIDIBuffers. There's one set of variables for keeping track of everything
  // in absolute time (when `isBeats` is False), and another for
  // relative-to-tempo timing (when note start times and durations are measured
  // in "quarter notes" (QN)).
 ...
 protected:
  ...
  BusesLayout makeBusesLayout(int inputs, int outputs) {
    ...
    const AudioChannelSet inputChannelSet =
        AudioChannelSet::discreteChannels(inputs);
    const AudioChannelSet outputChannelSet =
        AudioChannelSet::discreteChannels(outputs);
